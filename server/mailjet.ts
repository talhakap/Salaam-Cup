...
export async function sendTeamApprovedEmail(
  ...
  const subject = `Your Team "${teamName}" Has Been Approved - Salaam Cup`;

  const textPart = `Hi ${captainName},

Great news! Your team "${teamName}" has been approved for Salaam Cup.

You can log in to your captain dashboard to manage your team:
...
<div style="font-family: 'Nunito Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="padding: 30px 0;">
    ...
    <p style="font-size: 16px;">Great news! Your team <strong>"${teamName}"</strong> has been approved for Salaam Cup.</p>
    
    <p style="font-size: 16px;">Log in to your captain dashboard to manage your team roster and details.</p>
    
    <div style="text-align: center; margin: 30px 0;">
      <a href="${loginUrl}" style="background-color: #111; color: #fff; text-decoration: none; padding: 12px 32px; border-radius: 6px; font-weight: 700; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; display: inline-block;">Go to Dashboard</a>
    </div>
  </div>
