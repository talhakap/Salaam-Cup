...
- Playoffs/Bracket system: playoff_settings table (qualifyCount, bracketMode, seedingSource, reseedEachRound, locked, showBracket, generated) and playoff_matches table (round, matchIndex, seeds, teams, scores, status, venue, isBye, linkedMatchId); single-elimination bracket auto-generated from standings; proper seed ordering (1v8, 4v5, etc.); byes for non-power-of-2 team counts; winners auto-advance to next round; Admin Playoffs page (/admin/playoffs) for per-division setup with generate/reset; Public playoffs page (/tournaments/:id/playoffs) with bracket visualization per division; champion banner when final winner determined; placeholder match linking: imported placeholder matches (TBD teams) are linked to generated bracket matches by semantic round name (Finals→last round, Semis→second-to-last, Quarters→third-to-last), copying startTime/venueId/fieldLocation; venue/time/field info displayed on both public bracket and admin playoff cards
...
- Standings adjustments: `standings_adjustments` table for admin manual overrides (points, wins, losses, ties, goals for/against adjustments with notes); adjustments applied after base calculation in `recalculateStandings`, then re-sorted
- Standings reorder: Admin Standings page (/admin/standings-adjustments) with visual reorder interface; filter by tournament + division, see standings table with up/down arrows to manually reorder team positions; Save/Reset buttons; uses POST `/api/tournaments/:tid/standings/reorder` to persist positions
